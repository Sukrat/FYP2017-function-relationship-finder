...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \textbf{Result} subset of the data points that violate functional relation		
        \For{\textbf{each} $point$ \textbf{in} $dataset$}
        \For{\textbf{each} $neighbour$ \textbf{in} $dataset$}
        \If{$dist < tol$ and $|point.y - neighbour.y| > oTol$}
